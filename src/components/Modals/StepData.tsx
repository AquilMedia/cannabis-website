...
const PrescriptionWizardModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    // Determine which steps to show based on the answer to the first question
    let steps: StepData[] = [];
    if (answers[0] === true) {
    ...
    if (current.type === 'summary') {
        console.log('Prescription Wizard Answers:', answers, { prescriptionImg, identityImg });
    }
